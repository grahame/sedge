...
def ask_overwrite(file_name):
    ...
    confirm = input("Enter `yes' to overwrite: ")
    return confirm == 'yes'
    ...
    try:
        with open(file_name) as fd:
            ...
            if header.find(':sedge:') == -1:
                ...
                if okay:
                    backup_file = file_name + '.pre-sedge'
...
def init(config):
    ...
    if config_file.is_file():
        click.echo('{} already exists, maybe you want $ sedge update')
        sys.exit()
...
def update(config):
    ...
    with config_file.open() as fd:
        SedgeEngine(library, fd, not config.no_verify, url=config.config_file)
